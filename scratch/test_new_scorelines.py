...
# Add path so we can import packages
sys.path.append('.')

from fifa_predictor.knowledge import get_team_strength, RECENT_FORM
...
def new_predict_scoreline(team_a: str, team_b: str, pw: float, pl: float, outcome: str, match_date: date | None = None) -> tuple[int, int]:
...
cursor.execute("""
    SELECT match_number, kickoff_at, home_team, away_team, predicted_winner, home_win_probability, draw_probability, away_win_probability
...
for r in rows:
    match_num, kickoff_at, home, away, pred_winner, p_win, p_draw, p_loss = r
    ...
    if is_placeholder(home_clean) or is_placeholder(away_clean):
        ...
        pw_frac = p_win / 100.0
        pd_frac = p_draw / 100.0
        pl_frac = p_loss / 100.0
        
        if pw_frac >= pl_frac and pw_frac >= pd_frac:
            outcome = "win"
        elif pd_frac >= pw_frac and pd_frac >= pl_frac:
            outcome = "draw"
        else:
            outcome = "loss"
            
        score = new_predict_scoreline(home_clean, away_clean, pw_frac, pl_frac, outcome, md)
        
    score_str = f"{score[0]}-{score[1]}"
    score_counts[score_str] = score_counts.get(score_str, 0) + 1
    
    if match_num <= 30:
        sample_predictions.append((match_num, home_clean, away_clean, pred_winner, score_str))

print("\n--- New Scoreline Distribution ---")
for k, v in sorted(score_counts.items()):
    print(f"  {k}: {v}")

print("\n--- Sample of first 30 matches with new scorelines ---")
for s in sample_predictions:
